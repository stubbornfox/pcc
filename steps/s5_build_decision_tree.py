...
def predict_bird_by_activation(bird_activations, configuration):
  ...
  for i, e in enumerate(model.estimators_):
    if i in true_tree:
      a, weights = explore_tree(X_train, model.estimators_[i], n_nodes_[i], children_left_[i],
                       children_right_[i], feature_[i], threshold_[i],
                       suffix=i, sample_id=0, feature_names=feature_names)
      if (a > max_nodes) or (len(weights) > 0 and a == max_nodes and max(weights) > max_similarity):
        max_nodes = a
        min_i = i
