...
def resnet50_features_inat(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model_dict = torch.load(os.path.join(os.path.join('features', 'state_dicts'), 'full_bbn_pretrained_inat17_statedict.pth')) 
        # model_dict = torch.load(os.path.join(model_dir, 'full_bbn_pretrained_inat17_90epoch_statedict.pth'))
        
        # rename last residual block from cb_block to layer4.2
        new_model = copy.deepcopy(model_dict)
        for k in model_dict.keys():
            # print(k)
            if k.startswith('backbone.cb_block'):
                splitted = k.split('cb_block')
                # print("splitted: ", splitted)
                # print('layer4.2'+splitted[-1])
                new_model['layer4.2'+splitted[-1]]=model_dict[k]
                del new_model[k]
            elif k.startswith('backbone.rb_block'):
                del new_model[k]
