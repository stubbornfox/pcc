...
def draw_decision_path(image_id, dataset):
    ...
    cluster_nodes = [
        cluster_node(cluster_id, preview_image, weight=has_concept(weight))
    ...
    # class_nodes = [class_node(class_id, image_id) for class_id, image_id in related_class_ids]
    ...
    edges.append(edge(
        source=root_id,
        target=cluster_node_id(concept_ids[0]),
    ))
    for i in range(len(concept_ids) - 1):
        edges.append(edge(
            source=cluster_node_id(concept_ids[i]),
            target=cluster_node_id(concept_ids[i+1]),
        ))


    display_decision_tree(
        root_id=root_id,
        elements=[bird_node] + cluster_nodes + edges
    )
